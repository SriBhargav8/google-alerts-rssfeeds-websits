...
export default function EditIntegrationForm({ integration }: { integration: any }) {
  ...
  try {
    initialCredentials = JSON.parse(integration.credentials);
  } catch (e) {}
  ...
  const [categoryMappings, setCategoryMappings] = useState<any[]>(
    (initialCredentials as any).categoryMappings || []
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/integrations/${integration.id}`, {
        ...
        body: JSON.stringify({ name, credentials: { ...credentials, categoryMappings } }),
      });
  ...
  const getPlatformLabel = (t: string) => {
    switch(t) {
      case "wordpress": return "WordPress (REST API)";
      case "payloadcms": return "Payload CMS";
      ...
      <header className="flex flex-col md:flex-row md:justify-between md:items-start mb-8 space-y-4 md:space-y-0">
        <div>
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-3xl font-extrabold tracking-tight text-slate-900">Configure Integration</h1>
          </div>
          <p className="text-slate-500 mt-2 font-medium ml-9">Edit connection settings for {getPlatformLabel(type)}.</p>
        </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden flex flex-col">
        <div className="p-6 border-b border-slate-100 bg-slate-50/50 flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">Update the API credentials for this connection.</p>
          </div>
        </div>
        ...
        <div className="p-8 space-y-6">
          ...
          <div className="space-y-2">
            ...
            <input 
              disabled type="text" value={getPlatformLabel(type)}
              className="w-full px-4 py-3 border border-slate-200 rounded-lg bg-slate-100 text-slate-500 text-sm cursor-not-allowed"
            />
          </div>
