...
import { Plus, Trash2, Save, Rss, Send, Bot, Clock, ExternalLink, AlertTriangle, Trash, X } from "lucide-react";
import Link from "next/link";
import CronBuilder from "../../CronBuilder";

type Integration = { id: string; name: string; type: string; };

export default function EditWorkflowForm({ workflow, integrations, aiProviders = [] }: { workflow: any; integrations: Integration[], aiProviders?: { id: string, name: string, type?: string, modelName?: string, isDefault?: boolean }[] }) {
  const router = useRouter();
  const [name, setName] = useState(workflow.name);
  const [cronSchedule, setCronSchedule] = useState(workflow.cronSchedule);
  const [isActive, setIsActive] = useState(workflow.isActive);
  const [aiProviderId, setAiProviderId] = useState<string>(workflow.aiProviderId || "");
  const [feeds, setFeeds] = useState(workflow.feeds.length ? workflow.feeds.map((f: any) => ({ url: f.url, label: f.label })) : [{ url: "", label: "" }]);
  
  const initialIntegrationIds = workflow.destinations.map((d: any) => d.integrationId);
  
  const initialDestConfigs: Record<string, any> = {};
  workflow.destinations.forEach((d: any) => {
    if (d.config) {
      try {
        initialDestConfigs[d.integrationId] = JSON.parse(d.config);
      } catch(e) {}
    }
  });

  const [selectedIntegrations, setSelectedIntegrations] = useState<string[]>(initialIntegrationIds);
  const [destinationConfigs, setDestinationConfigs] = useState<Record<string, any>>(initialDestConfigs);
  const [logoUrl, setLogoUrl] = useState<string>(workflow.logoUrl || "");
  const [systemPrompt, setSystemPrompt] = useState<string>(workflow.systemPrompt || "");
  const [generateImages, setGenerateImages] = useState<boolean>(workflow.generateImages !== undefined ? workflow.generateImages : false);
  const [cmsContentFormat, setCmsContentFormat] = useState<string>(workflow.cmsContentFormat || "LEXICAL");
  const [includeSourceLink, setIncludeSourceLink] = useState<boolean>(workflow.includeSourceLink !== undefined ? workflow.includeSourceLink : false);
  const [scrapeFullContent, setScrapeFullContent] = useState<boolean>(workflow.scrapeFullContent ?? false);
  const [generationMode, setGenerationMode] = useState<string>(workflow.generationMode || "ROUNDUP");
  const [maxIndividualPosts, setMaxIndividualPosts] = useState<number>(workflow.maxIndividualPosts || 5);
  const [enableStrictFiltering, setEnableStrictFiltering] = useState<boolean>(workflow.enableStrictFiltering ?? true);
  const [useNofollowLinks, setUseNofollowLinks] = useState<boolean>(workflow.useNofollowLinks ?? true);
  ...
  const handleRemoveFeed = (index: number) => setFeeds(feeds.filter((_: unknown, i: number) => i !== index));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    const validFeeds = feeds.filter((f: any) => f.url.trim() !== "");
    ...
    try {
      const res = await fetch(`/api/workflows/${workflow.id}`, {
        ...
        body: JSON.stringify({ 
          name, cronSchedule, isActive, feeds: validFeeds, integrationIds: selectedIntegrations, 
          destinationConfigs, aiProviderId, logoUrl, systemPrompt, generateImages, 
          cmsContentFormat, includeSourceLink, scrapeFullContent,
  ...
  const executeDelete = async () => {
    if (deleteConfirmText !== workflow.name) return;
  ...
  return (
    <>
      <form onSubmit={handleSubmit} className="w-full max-w-7xl mx-auto pb-12">
        {/* Header */}
        <header className="flex flex-col md:flex-row md:justify-between md:items-center space-y-4 md:space-y-0 mb-8 pb-6 border-b border-slate-200">
          <div>
            <h1 className="text-3xl font-extrabold tracking-tight text-slate-900">Edit Workflow</h1>
            <p className="text-slate-500 mt-1 font-medium">Update your automated content pipeline.</p>
          </div>
          <div className="flex flex-wrap items-center gap-2 sm:gap-3">
            <button 
              type="button" 
              onClick={() => setShowDeleteModal(true)} 
              className="px-4 py-2 bg-red-50 text-red-700 hover:bg-red-600 hover:text-white border border-red-200 hover:border-red-600 rounded-lg text-sm font-bold transition-all shadow-sm flex items-center space-x-2"
            >
              <Trash size={14} />
              <span>Delete</span>
            </button>
            ...
            >
              <Save size={16} />
              <span>{loading ? "Saving..." : "Save Workflow"}</span>
            </button>
          </div>
        </header>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
              <div className="p-4 border-b border-slate-100 bg-slate-50/50 flex items-center space-x-3">
                ...
                <h2 className="text-base font-bold text-slate-900">1. Feed Sources</h2>
              </div>
            ...
            <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
              <div className="p-4 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
                <div className="flex items-center space-x-3">
                  ...
                  <h2 className="text-base font-bold text-slate-900">2. Configuration & AI</h2>
                </div>
                <label className="flex items-center space-x-2 cursor-pointer bg-white px-3 py-1 rounded-md border border-slate-200 shadow-sm">
                  <span className={`text-xs font-bold tracking-wide ${isActive ? "text-emerald-600" : "text-slate-500"}`}>
                    {isActive ? "ACTIVE" : "PAUSED"}
                  </span>
                  <input type="checkbox" checked={isActive} onChange={(e) => setIsActive(e.target.checked)} className="w-4 h-4 text-emerald-600 rounded focus:ring-emerald-500 border-slate-300" />
                </label>
              </div>
                ...
                {integrations.length === 0 ? (
                  ...
                  integrations.map((integration) => {
                    ...
                    <div key={integration.id} className={`relative flex flex-col p-4 border rounded-lg transition-all ${
                      ...
                      {isChecked && (integration as any).categoryMappings && (integration as any).categoryMappings.length > 0 && (
                        <div className="mt-4 ml-7">
                          ...
                          >
                            ...
                            {(integration as any).categoryMappings.map((map: any, idx: number) => (
                              <option key={idx} value={map.name}>{map.name}</option>
                            ))}
                          </select>
                        </div>
                      )}
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-lg p-4 flex items-start space-x-3 mt-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-bold text-slate-900 mt-0.5">the cron expression above.</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-slate-100 flex justify-between items-start">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  <h3 className="text-xl font-extrabold text-slate-900">Delete Workflow</h3>
                  <p className="text-sm font-medium text-red-600 mt-1">This action cannot be undone.</p>
                </div>
              </div>
              <button onClick={() => setShowDeleteModal(false)} className="text-slate-400 hover:text-slate-600 bg-slate-50 hover:bg-slate-100 p-2 rounded-full transition-colors">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="p-6 bg-slate-50 space-y-4">
              <p className="text-slate-700 text-sm">
                You are about to completely delete this workflow. <strong>All connected integrations with this flow will be disconnected</strong>, and the historical run logs will be permanently erased.
              </p>
              
              <div className="space-y-2 mt-4">
                <label className="text-sm font-bold text-slate-700">
                  Please type <span className="text-slate-900 font-black bg-white px-1.5 py-0.5 rounded border border-slate-200">{workflow.name}</span> to confirm.
                </label>
                <input 
                  type="text" 
            ...
            <div className="p-6 bg-white border-t border-slate-100 flex justify-end space-x-3">
              ...
              <button 
                ...
                disabled={deleteConfirmText !== workflow.name || loading}
                className="px-6 py-2.5 text-sm font-bold text-white bg-red-600 hover:bg-red-700 disabled:bg-red-300 disabled:cursor-not-allowed rounded-xl transition-colors shadow-sm flex items-center space-x-2"
              >
